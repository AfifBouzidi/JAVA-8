...
public class JavaEightTest {
	...
	public void test() {
		LOG.info("######Predicate#########");
		LOG.info(mobileService.filter(mobiles, (Mobile m) -> m.getPrice() > 60).toString());
		LOG.info("######Consumer#########");
		mobileService.forEach(mobiles, (Mobile m) -> LOG.info(m.getMobileType().name()));
		LOG.info("######Function#########");
		LOG.info(mobileService.map(mobiles, (Mobile m) -> m.getMobileType()).toString());
	}
